...
red_wire_path = mark_path(red_wire)
green_wire_path = mark_path(green_wire)

def find_min_manhattan_path(path_one, path_two)
    intersection_points_sets = path_one.to_set & path_two.to_set
    intersection_points_arr = intersection_points_sets.to_a

    min_path = 0
    min_path_index = 0

    intersection_points_arr.each_with_index do |points, i|
        x, y = points[0], points[1]
        path = x.abs + y.abs
        if min_path == 0 || min_path > path
            min_path = path
            min_path_index = i
        end
    end
    return intersection_points_arr[min_path_index];
end

def find_min_steps(path_one, path_two)
    intersection_points_sets = path_one.to_set & path_two.to_set
    intersection_points_arr = intersection_points_sets.to_a

    path_one_intersect = find_pos_hash_index(path_one, intersection_points_arr)
    path_two_intersect = find_pos_hash_index(path_two, intersection_points_arr)

    min_steps = 0
    path_one_intersect.each do |position_one, steps_one|
        steps = steps_one + path_two_intersect[position_one]
...
def find_pos_hash_index(path, intersection_points_arr)

    path_hash = {}

    path.each_with_index do |position, idx|
        if intersection_points_arr.include?(position) && !path_hash.key?(position)
            path_hash[position] = idx + 1
        end
    end
...
# p find_min_manhattan_path(red_wire_path, green_wire_path)
p find_min_steps(red_wire_path, green_wire_path)